...
class MATGRN(nn.Module):
    def __init__(self, num_nodes, input_dim, output_dim, horizon, rnn_units, num_layers=1, cheb_k=3,
        ...
        self.mem_spatial = MemoryAugmented(num_nodes=self.num_nodes, mem_num=self.num_nodes, mem_dim=self.mem_dim, fusion_factor=self.fusion_factor)
        self.mem_temporal = MemoryAugmented(num_nodes=self.num_nodes, mem_num=self.mem_num, mem_dim=self.mem_dim, fusion_factor=self.fusion_factor)
        # self.conv = SpatioTemporalConv(self.model_dim, self.model_dim, 3, 3)
        # 寻找图特征
        ...
        self.decoder = AGCRNN_Decoder(self.num_nodes, self.output_dim + self.ycov_dim + self.model_dim,
                                      self.decoder_dim,
    ...
    def forward(self, x, x_cov, y_cov, labels=None, batches_seen=None):
        ...
        # node_embeddings1 = torch.matmul(self.mem_spatial.memory['Memory'], self.mem_temporal.memory['Memory'].T)
        # node_embeddings2 = torch.matmul(self.mem_spatial.memory['Memory'], self.mem_temporal.memory['Memory'].T)
        ...
        # x_fusion = x_temporal + x_spatial
        # Decoder
        h_t = self.reg(x_temporal + x_spatial)[:, 0, ...]
